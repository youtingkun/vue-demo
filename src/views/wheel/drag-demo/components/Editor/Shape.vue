	...
	methods: {
		...
		handleRotate(e) {
			...
			const move = moveEvent => {
				...
				// 修改当前组件样式
				this.$store.commit('dragDemo/setShapeStyle', pos);
			};

			const up = () => {
				hasMove && this.$store.commit('dragDemo/recordSnapshot');
		...
		handleMouseDownOnShape(e) {
			console.log('handleMouseDownOnShape事件');
			if (this.element.component != 'v-text') {
				e.preventDefault();
			}

			e.stopPropagation();
			this.$store.commit('dragDemo/setCurComponent', { component: this.element, index: this.index });
			this.cursors = this.getCursor(); // 根据旋转角度获取光标位置
		...
		handleMouseDownOnPoint(point, e) {
			console.log('handleMouseDownOnPoint事件');
			...
			const move = moveEvent => {
				...
				this.$store.commit('dragDemo/setShapeStyle', style);
			};

			const up = () => {
				document.removeEventListener('mousemove', move);
				document.removeEventListener('mouseup', up);
				needSave && this.$store.commit('dragDemo/recordSnapshot');
			};
